...
def main():
    ...
    parser.add_argument("--bangla_csv", type=str, default=None, help="Path to Bangla lyrics CSV")
    parser.add_argument("--tfidf-features", type=int, default=5000, help="Max TF-IDF features for lyrics")
    ...
    if args.bangla_csv is not None:
        ...
        y_true = y_lang  # use language/category labels for evaluation

        # Disable audio for Bangla dataset by default
        args.use_audio = False
        print(f"Loaded Bangla dataset: {X.shape[0]} samples, {X.shape[1]} features")

    else:
        # ----------------- Hybrid dataset (audio + lyrics) ----------------- #
    ...
    # 5) ConvVAE on spectrograms (optional)
    if args.spectrogram_dir and args.use_audio:
